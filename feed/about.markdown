---
layout: page
title: О себе
feed-type: about
permalink: /feed/about
---

<style>
body {
   	   background: ##f1f1f1;
   	   text-align: center;
}
img {
    text-align: center;
    border-radius: 28px;
    box-shadow: 4px 4px 14px black;
}
p {
    text-align: center;
} 
</style>

<a href="/assets/images/I.jpg" target="_blank">
    <img src="/assets/images/I.jpg" alt="I'm">
</a>

<p>Родился 22.01.1962г. На Южном Урале, г.Челябинск./

Пенсионер./

Более 40-ка проживаю на Камчатке, г.Петропавловск-Камчатский./

Увлечения: Unix. Ну и все что с этим связано./

Предпочитаемые дистрибутивы: FreeBSD, Slackware, Gentoo./

Из наших, Российских: УльяновскBSD, Calculate./

Свободное ПО.</p>


