...
Инструкции `\userpackage` загружают предустановленные пакеты `LaTex`, которые используются для загрузки шрифтов, поддержки кодировки и качество шрифта.

Пакеты `amsmath`,`amsthm`,`amssymb` нужны для работы кириллицы, пакет `fontenc` загружает кодировки шрифтов, `T1` для латиницы и `T2A` для кириллицы. 
