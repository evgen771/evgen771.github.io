...
#### **Устанавливал с помощью Live CD/DVD Linux Mint 22.2**

***Все параметры даны для моей машины!!! Будьте внимательны при создании базового `make.conf`!!!***
...
Я использую `Stage3 с OpenRC` и готовым desktop профилем.

Вы можете выбрать любой архив под ваши задачи и нужды. 

ПКМ по профилю и скопируем ссылку для скачивания. 
...
Тут всё зависит от ресурсов вашего компьютера.

 Одна задача занимает около 2 гигабайт оперативной памяти. 

Следовательно, флаг "-j2" будет занимать 4 ГБ ОЗУ и так далее. 

В моем случае я возьму "-j10" как самый средний вариант.:
...
USE="dri X xorg dbus nls udev pulseaudio pavucontrol alsa session libnotify policykit svg jpeg gconf gtk gtk3 $USE_ARCHIVES $USE_CPUFLAGS_LEGACY -wayland -telemetry -gnome -kde -minimal -qt"
MAKEOPTS="-j10"
GRUB_PLATFORMS="pc"
...
Локали, как и в арче, находятся в `/etc/locale.gen` 

Однако вместо списка там надо будет написать свои.
...
#### Terminal (необязательно, чего нет в `xfce-base/xfce4-meta` можно установить после установки системы)...
...
Лечится так,смотреть ниже(предупреждение):
