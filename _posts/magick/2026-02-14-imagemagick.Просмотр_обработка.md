...
- [Пакет ImageMagick входит почти в каждый дистрибутив Linux](#пакет-imagemagick-входит-почти-в-каждый-дистрибутив-linux)
- [Утилиты ImageMagick и их использование](#утилиты-imagemagick-и-их-использование)
- [display](#display)
- [Просмотр графического файла из mc (Midnight Commander)](#просмотр-графического-файла-из-mc-midnight-commander)
- [Функции кнопок мыши](#функции-кнопок-мыши)
- [Создание снимка области экрана](#создание-снимка-области-экрана)
- [Из контекстного меню display](#из-контекстного-меню-display)
- [Получение информации об изображении](#получение-информации-об-изображении)
- [Вызов окна с гистограммой изображения](#вызов-окна-с-гистограммой-изображения)
- [Возможно использование "горячих" клавиш в работе](#возможно-использование-горячих-клавиш-в-работе)
- [display в качестве консольной утилиты](#display-в-качестве-консольной-утилиты)
- [convert](#convert)
- [identify](#identify)
- [import](#import)
- [animate](#animate)
...
С `ImageMagick` создаются динамические изображения для Web страниц:
    ...
    производится трансформация изображений: 
    изменение размеров, цвета, вращение, создаются надписи на изображении и комментарии получают снимки экрана, окна, выделенной области добавляются спецэффекты для искажения изображения используют и сохраняют изображения всех популярных форматов TIFF, JPEG, PNG, PDF, PhotoCD, GIF...
...
находясь в каталоге с отмеченными файлом, вызвать меню `F2` и выбрать `Do something with current file`. 

В окне ввести `display`. 

Откроется окно `ImageMagick` с изображением из отмеченного файла.
...
Указать задержку, после которой создается снимок => oпять нажать `Grab`.

Курсор изменяет форму. 

Обвести область на экране и щелчком получить ее снимок.
...
Левой кнопкой - на окне display. 

Откроется Commands - меню и редактор изображений.
...
Изменение формата файла с `*.jpg` на `*.bmp`. 

Тип файла распознается программой по расширению:
...
На изображении `magic.jpg` от точки `100*100` сделать надпись `"IMAGE"` синим цветом, шрифтом `helvetic`. 

Результат записать в `I_magic.jpg`:
...
запись образа окна, выбранной области экрана в файл. 

Имеется около 20 опций.
...
Сделать снимок окна с записью в `shot.bmp`. 

Окно выбирается одинарным щелчком под крестиком-курсором:
...
Показ слайд-шоу из файлов по маске в текущем каталоге с интервалом просмотра 100 сотых долях секунды (здесь - одна секунда):
...
трансформация изображения или их последовательности (без применения скриптов). 
