...
#### Вполовину уменьшить размер картинки, то введите команду:
...
#### Преобразовать JPG-изображение в формат PNG, введите и примените команду:
...
Итогом ее применения станут краткие данные о картинке. 

---

#### Узнать о ней больше вам поможет уже другая команда:
...
#### Получить полный перечень всех используемых в изображении цветов:
...
Хотите поместить картинку в рамку:

`convert -bordercolor red -border 10 1.jpg 1_3.png`

Цвет, в данном случае `«red» (красный)`, указывается на английском.

Цифра, в рассматриваемом примере `«10»`, определяет толщину рамки. 

Результатом применения команды стал графический файл `«1_3.png»`.

---

#### Сделать из картинки негатив:

`convert -negate 1.jpg 1_4.jpg`

---

#### Сделать черно-белое (монохромное):
...
В том же каталоге, в котором располагается ваши изображения, появится файл формата PDF. 

---

#### Графический интерфейс, введите команду:

`display 1.jpg`

*Файл появится в окне графического редактора с достаточно большим набором функций по редактированию изображений и подборкой эффектов. Этот инструмент полезен тем, кто не располагает мощным компьютером*.
