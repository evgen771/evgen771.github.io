---
layout: post
title: ImageMagick. Пакетная обработка изображений 2
categories: Magick
---

Можно модифицировать скрипт так, чтобы он изменял размер изображений с любых поддерживаемых форматов (`.jpg, .jpeg, .png, .webp, .gif`) и сохранял результат в формате `.jpg`.
...
прозрачный фон будет заменён на чёрный или белый (по умолчанию — чёрный). 

Чтобы задать белый фон:
