...
- [Создание миниатюр графических файлов для приложений KDE](#создание-миниатюр-графических-файлов-для-приложений-kde)
- [Основные инструменты ffmpeg:](#основные-инструменты-ffmpeg)
- [Основная форма команды ffmpeg:](#основная-форма-команды-ffmpeg)
- [Получить информацию медиа файла](#получить-информацию-медиа-файла)
- [Конвертировать медиа файлы](#конвертировать-медиа-файлы)
- [Извлечение аудио из видео](#извлечение-аудио-из-видео)
- [Отключение звука в видео](#отключение-звука-в-видео)
- [Извлечение изображений из видео](#извлечение-изображений-из-видео)
- [Изменение разрешения видео или соотношения сторон](#изменение-разрешения-видео-или-соотношения-сторон)
- [Добавить изображение обложки в аудио](#добавить-изображение-обложки-в-аудио)
- [Добавить субтитры к видео](#добавить-субтитры-к-видео)
- [Сжатие медиа-файлов](#сжатие-медиа-файлов)
- [Обрезать медиа файлы](#обрезать-медиа-файлы)
- [Обрезка медиа-файлов](#обрезка-медиа-файлов)
- [Склейка медиа-файлов](#склейка-медиа-файлов)
- [Соедините изображения в видео](#соедините-изображения-в-видео)
- [Запись экрана](#запись-экрана)
- [Запишите свою веб-камеру](#запишите-свою-веб-камеру)
- [Запишите ваше аудио](#запишите-ваше-аудио)
- [Основное использование фильтров в ffmpeg](#основное-использование-фильтров-в-ffmpeg)
- [Масштабирование видео](#масштабирование-видео)
- [Обрезка видео](#обрезка-видео)
- [Поворот видео](#поворот-видео)
- [Преобразование аудиоканала](#преобразование-аудиоканала)
- [Увеличение громкости звука](#увеличение-громкости-звука)
- [Настройка скорости воспроизведения](#настройка-скорости-воспроизведения)
- [Добавить водяной знак](#добавить-водяной-знак)
...
Вы можете установить флаг `-crf` (C`onstant Rate Factor` - коэффициент постоянной скорости). Чем ниже `CRF`, тем выше скорость передачи данных. Также помогает использование `libx264` в качестве видеокодека. 
...
Чтобы сделать это, разделите количество изображений на продолжительность аудио (в секундах). 

Для нашего примера у нас есть аудиофайл длиной 22 секунды и 9 изображений. 9 разделить на 22 составляет приблизительно 0,40, поэтому мы будем использовать это для нашей частоты кадров:

`cat my_photos/* | ffmpeg -framerate 0.40 -f image2pipe -i - -i audio.wav -c copy video.mkv`

### Запись экрана

Тут нужно использовать формат `-f x11grab`. Это запишет ваш `XSERVER`. В качестве входных данных вы должны будете указать номер вашего экрана (основной экран обычно должен быть `0:0`). Но это будет захватывать только верхнюю левую часть экрана. Вы должны добавить размер экрана (или экранов). Наш размер - `1920x1080`. 
...
Первая команда повернет видео по часовой стрелке на 90 градусов. Вторая команда перевернет видео вверх ногами (`PI  = 180 градусов`).
...
Фильтр для аудио `- atempo`. Есть одно маленькое предостережение: оно может принимать значения только от `0,5` (половина скорости) до `2 `(удвоение скорости). 
...
Но удобнее использовать формулу `overlay=(main_w-overlay_w)/2`:

`(main_h-overlay_h)/2`, где `main_w` и `main_h` - высота и ширина видео, а `overlay_w` и `overlay_h` - водяного знака. 
